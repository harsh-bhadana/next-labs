...
const generateInitialData = (random = false): SensorData[] => 
  Array.from({ length: GRID_SIZE }, (_, i) => ({
    id: i,
    value: random ? Math.random() * 100 : 0, // Deterministic 0 for initial hydration
    status: 'active',
    trend: 'stable'
  }));
...
export default function MemoFreeLab() {
  ...
  useEffect(() => {
    ...
    const loop = (time: number) => {
      ...
      if (time - lastTick >= 16) {
        ...
        setSensors(prev => {
          const next = [...prev];
          // Optimization: Only update 16 sensors per frame (25% of grid)
          // This rotates the "active" nodes to maintain the illusion of total real-time
          // while drastically reducing the React reconciliation workload per frame.
          for (let i = 0; i < 16; i++) {
          ...
          batchOffset = (batchOffset + 16) % GRID_SIZE;
          return next;
        });
  ...
  // --- HANDLERS (Intentional: No useCallback) ---
  const handleReset = () => {
      ...
      <main className="relative z-10 flex-1 grid grid-cols-1 lg:grid-cols-[1fr_350px] gap-px bg-zinc-900 h-[calc(100vh-65px)]">
        ...
        <section className="bg-zinc-950 p-6 overflow-y-auto custom-scrollbar">
          <div className="grid grid-cols-2 sm:grid-cols-4 md:grid-cols-6 xl:grid-cols-8 gap-3">
            {sensors.map((sensor) => (
              // Child Component (Intentional: No React.memo)
              <SensorCard 
                key={sensor.id} 
                sensor={sensor} 
                getStatusColor={getStatusColor} // Intentional: Passing raw function
              />
            ))}
          </div>
        </section>
                 ...
                 <div key={s.id} className="flex items-center justify-between p-3 rounded-xl bg-zinc-900/30 border border-zinc-800/50">
                    ...
                    <div className="flex items-center gap-3">
                       <div className="w-12 h-1 bg-zinc-800 rounded-full overflow-hidden">
                          <div className="h-full bg-blue-500 transition-all duration-300" style={{ width: `${s.value}%` }}></div>
                       </div>
...
function SensorCard({ sensor, getStatusColor }: { sensor: SensorData, getStatusColor: (s: string) => string }) {
  const colorClass = getStatusColor(sensor.status);
  
  return (
    <div className={`p-4 rounded-2xl border transition-all duration-300 flex flex-col gap-4 group hover:ring-2 hover:ring-blue-500/50 ${colorClass} bg-black/40`}>
